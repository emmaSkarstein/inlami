...
pumpkins <- pumpkins_original %>% 
  ...
  mutate(across(c("weight_lbs", "ott"), ~ . - mean(.))) %>% 
  mutate(ott2 = ott^2) %>% 
...
pumpkin_model_lm <- lm(weight_lbs ~ ott + year + type, data = pumpkins)
...
library(inlami)
inla_pumpkin <- inla(
  formula = weight_lbs ~ ott + year + Giant_Pumpkin + Giant_Squash + Giant_Watermelon, 
  data = pumpkins,
  family = "gaussian")

summary(inla_pumpkin)

pumpkin_model <- fit_inlami(
  formula_moi = weight_lbs ~ ott + Giant_Pumpkin + Giant_Squash + Giant_Watermelon,
  formula_imp = ott ~ Giant_Pumpkin + Giant_Squash + Giant_Watermelon,
  family_moi = "gaussian",
  data = pumpkins,
  error_type = "missing",
  prior.prec.moi = c(2, 1),
