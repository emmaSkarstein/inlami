...
#'
#' #simplify_inlami_model_summary(simple_model)
...
#' Summarise INLA measurement error and missing data model
#'
#' @param inlami_model the model returned from the fit_inlami function.
#'
#' @return Prints a structured summary tailored to clarify the different components of the measurement error/missing data model.
#' @export
#'
#' @examples
#' simple_moi <- y ~ x + z
...
summarise_me <- function(inlami_model){
  ...
  print(simple_summary$moi_coef)
  cat("\n")

  cat("Coefficient for error prone variable: \n")
  print(simple_summary$error_coef)
  cat("\n")

  cat("Fixed effects for imputation model: \n")
  print(simple_summary$imp_coef)
  cat("\n")

  cat(paste0("Model hyperparameters (apart from ", rownames(simple_summary$error_coef), "): \n"))
  print(simple_summary$other_model_hyperpar)
  cat("\n")
